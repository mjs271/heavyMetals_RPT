...
! kappas are weights that determine the partitioning of diffusion between
! random walks (kappaD), mass-transfer (kappaM), and, within mass-transfer,
! the immobile-to-mobile direction (kappaI) and the mobile-to-immobile direction (kappaM)
double precision, parameter :: kappaD = 0.5d0, kappa = (1.0d0 - kappaD),&
                               kappaM = 0.5d0, kappaI = 1.0d0 - kappaM
integer, parameter          :: nspec = 20, numsd = 5, num_alloc = 4e3
...
subroutine mobile2immobile(ip, mp, na, alive, ni, D, dt, nc, Distmat)
    ...
    double precision, intent(in   ) :: D(:), dt, Distmat(ni, na)
        ! diffusion coefficient array, time step, and domain length
    integer                         :: i, j
    double precision                :: WMmat(ni, na), denom(ni, na), colsum(na)
    ! denom is used in v(s) calculation but is pre-calculated for efficiency
    ! v_s is encounter density for a mobile/immobile particle pair
...
! quicksort subroutine--sorts a vector into ascending order
! source: https://gist.github.com/t-nissie/479f0f16966925fa29ea#file-sortreal8-f-L4
